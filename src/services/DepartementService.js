...
class DepartementService {
  constructor() {
    ...
    this._baseCacheKey = 'departements';
  }
  ...
  async _invalidateDepartementCache(id = null) {
    const keys = [
  ...
  async create({ name, created_by }) {
    const query = {
      text: `
        INSERT INTO departement (name, created_by)
        VALUES ($1, $2)
        RETURNING *
      `,
    ...
    await this._invalidateDepartementCache();
  ...
  async update(id, { name, updated_by }) {
    ...
    await this._invalidateDepartementCache(id);
