...
class StatisticsService {
    ...
    async refreshMaterializedViews() {
        try {
            ...
            // Log success here if necessary or notify
        } catch (error) {
            throw new ClientError('Error refreshing materialized views', error); // Throw error as ClientError
        }
    }
    ...
    async getDashboardSummary() {
        ...
        `;
            ...
            throw new ClientError('Error getting dashboard summary', error); // Throw error as ClientError
        }
    }
    ...
    async getAppointmentTrends(days = 30) {
        try {
            ...
            throw new ClientError('Error getting appointment trends', error); // Throw error as ClientError
        }
    }
    ...
    async getMonthlyAppointmentStats() {
        try {
            const query = `
            SELECT
                TO_CHAR(created_at, 'YYYY-MM') AS month,  -- Format Year-Month (e.g., 2023-01)
                COUNT(*) AS total_appointments
            FROM janji_temu
            WHERE deleted_at IS NULL
            GROUP BY TO_CHAR(created_at, 'YYYY-MM')
            ORDER BY TO_CHAR(created_at, 'YYYY-MM') DESC;  -- Sort by most recent first
        `;
    ...
    async getCounselorPerformance(limit = 5) {
        try {
            ...
            throw new ClientError('Error getting counselor performance', error); // Throw error as ClientError
        }
    }
    ...
    async getConsultationTypesDistribution() {
        try {
            ...
            throw new ClientError('Error getting consultation types distribution', error); // Throw error as ClientError
        }
    }
    ...
    async getDepartmentStats() {
        ...
        `;
            ...
            throw new ClientError('Error getting department stats', error); // Throw error as ClientError
        }
    }

    // Fungsi untuk mendapatkan data pengajuan per prodi jenjang
    ...
    // Fungsi untuk mendapatkan demografi mahasiswa per prodi
